...
// Remember, for reasons that none of us understand, we're using the RESULT_OK flag to pass back and forth the status of the MainActivity. So, we only want this next bit of code to execute if the MainActivity is not up and running and therefore is not RESULT_OK. (or something like that....just trust me here). 
